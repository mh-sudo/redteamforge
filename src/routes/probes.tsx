...
export const Route = createFileRoute("/probes")({ component: ProbesPage });
...
function ProbesPage() {
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-3 max-w-xl text-sm text-muted">
            {PROBES.length} Garak-style probes mapped to OWASP LLM Top 10 and MITRE ATLAS.
            Payloads are educational. They target the demo policy, not the public internet.
          </p>
        </div>
        <Button asChild>
      ...
      <p className="text-xs text-subtle">{list.length} probes</p>
          ...
          <Card key={probe.id} className="p-5">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <p className="mt-1 text-sm text-muted">{probe.description}</p>
              </div>
